"""
╔══════════════════════════════════════════════════════════════╗
║           US30 (DOW JONES) SCALPER v1.0                     ║
║    Same v1.5.3 strategy as Gold — adapted for US30           ║
║    ATR×2.5 | RR 1:2.0 | TP1 0.5R | 67% partial             ║
║    Backtest: WR 70.9% | PF 1.24 | +$5,416                   ║
╚══════════════════════════════════════════════════════════════╝
"""
...
@dataclass
class ScalpConfig:
    """Configuration tuned for US30 (Dow Jones) scalping."""
    ...
    # ─── Symbol ───────────────────────────────────────────────────
    SYMBOL: str = "US30"              # ICMarkets US30
    POINT: float = 0.01
    ...
    # ─── Risk Management ─────────────────────────────────────────
    RISK_PERCENT: float = 1.0         # start conservative on new pair
    ...
    # ─── Spread Filter ────────────────────────────────────────────
    MAX_SPREAD_POINTS: float = 5.0     # US30 wider spread
    SPREAD_CHECK_ENABLED: bool = True

    # ─── Scalp SL/TP (US30 adapted) ─────────────────────────────
    ATR_PERIOD: int = 10
    ATR_SL_MULTIPLIER: float = 2.5
    MIN_SL_POINTS: float = 15.0        # US30: min 15 points
    MAX_SL_POINTS: float = 80.0        # US30: max 80 points
    ...
    # ─── Round Numbers ────────────────────────────────────────────
    ROUND_NUMBER_INTERVAL: float = 500.0    # US30: 500-point levels
    ROUND_NUMBER_ZONE: float = 20.0         # wider zone for indices
...
def setup_logging() -> logging.Logger:
    ...
    fh = logging.FileHandler("us30_scalper.log", encoding="utf-8")
    fh.setFormatter(fmt)
...
class PositionMgr:
    ...
    def calc_lots(self, sl_dist: float) -> float:
        ...
        per_lot = 1.0  # US30: $1 per point per lot
        if sl_dist <= 0:
            return 0.01
        lots = risk / (sl_dist * per_lot)
        return max(0.01, min(round(lots, 2), 5.0))  # US30 allows bigger lots
                    ...
                    if t.pnl == 0:
                        try:
                            ...
                            if close_price > 0 and t.lots > 0:
                                if t.direction == Direction.LONG:
                                    t.pnl = (close_price - t.entry) * t.lots * 1  # US30: $1/point/lot
                                else:
                                    t.pnl = (t.entry - close_price) * t.lots * 1  # US30: $1/point/lot
